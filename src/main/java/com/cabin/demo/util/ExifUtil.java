...
public class ExifUtil {
    ...
    public static ExifData getExifData(byte[] fileContent) throws Exception {
        ...
        StringBuilder exifEntry = new StringBuilder();
        Iterable<Directory> directories = metadata.getDirectories();
        for (Directory dir : directories) {
//            exifEntry.append(dir.getName()).append("\n");
            for (Tag tag : dir.getTags()) {
                exifEntry.append(tag).append("\n");
            }
        }
        result.setExifEntry(exifEntry.toString());
            ...
            for (Tag tag : dir.getTags()) {
                ...
                switch (type) {
                    ...
                    case TAG_FNUMBER:
                        ...
                        result.setFNumber(Double.valueOf(desc.replaceFirst("f/", "")));
                        break;
                    case TAG_FOCAL_LENGTH:
                        // desc like "35.0 mm"
                        result.setFocalLength(Double.valueOf(
                                desc.replaceAll("[^0-9\\.]", "")
                        ));
                        break;
                    case TAG_FLASH:
